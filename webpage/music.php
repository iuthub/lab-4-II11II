...
?>
		...
		<div id="listarea">
			<ul id="musiclist">
				...
				if($playlist)
				{
					$musics = file("songs/$playlist", FILE_IGNORE_NEW_LINES);
				}
				else if($shuffle)
				{
				...
				foreach ( $musics as $music) {
					if(strstr($music, ".mp3")){
					?>
					<li class="mp3item" ><a href="<?= 'songs/'.basename($music)?> "> <?= basename($music) ?></a> (<?= songSize(filesize("songs/". basename($music))) ?>)</li>
				<?php } }
